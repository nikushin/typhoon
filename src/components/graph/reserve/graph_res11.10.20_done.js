...
import {SVGgraph, SVGadditional, SVGbackground, SVGcontainer, SaveRuqest} from "./styled-components"

const Graph = ({parameters = {}}) => {

  const { VirtualWidth = 960, VirtualHeight = 400, Width = 900, Height = 500,
  ...
  const graph_path_first = useSelector(state => state.graphKeeper.graph_path_first);
  const graph_path_second = useSelector(state => state.graphKeeper.graph_path_second);
  ...
  const graph_path_first_fill = graph_path_first + "V 0 H " + graph_start_time;
  const graph_path_second_fill = graph_path_second + "V 0 H " + graph_start_time;
