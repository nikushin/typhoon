...
const Graph = ({parameters = {}}) => {
  ...
  const { DefaultWidth = 900, DefaultHeight = 400 } = parameters;
  ...
  useEffect(() => {
    ...
    if (ViewBoxParameters.globalY < MaxY && ViewBoxParameters.height <= DefaultHeight) {
...
const Wheel = (e) => {
  ...
  SetViewBoxParameters((s) => ({ ...s,
    globalX: e.deltaY < 0 ? s.globalX + (s.width - DefaultWidth*s.scale)*(e.clientX/DefaultWidth) :
      s.globalX + (s.width - DefaultWidth*s.scale)/2,
    globalY: e.deltaY < 0 ? s.globalY + (s.height - DefaultHeight*s.scale)*(e.clientY/DefaultHeight) :
      s.globalY + (s.height - DefaultHeight*s.scale)/2,
    width : DefaultWidth*s.scale,
    height : DefaultHeight*s.scale,
  }));
  };
  ...
  const MouseMove = (e) => {
    ...
    if (ViewBoxParameters.movable) {
      SetViewBoxParameters((s) => ({ ...s,
        ...
        (s.height <= DefaultHeight ? MaxY : s.globalY) : //если большой масштаб - ничего не делаем, т.к. вылезаем полюбому
  ...
  return (
    <Fragment>
      ...
      <svg className="graph" height={DefaultHeight} width={DefaultWidth} viewBox={
